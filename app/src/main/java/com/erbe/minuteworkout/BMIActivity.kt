...
class BMIActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        calculate_units_btn.setOnClickListener {
            if (currentVisibleView.equals(METRIC_UNITS_VIEW)) {
                if (validateMetricsUnit()) {

                    val heightValue: Float = metric_unit_height_et.text.toString().toFloat() / 100
        ...
        units_rg.setOnCheckedChangeListener { radioGroup, i ->
            if (i == R.id.metric_unit_rb) {
                makeVisibleMetricUnitsView()
            }
            else {
    ...
    private fun validateMetricsUnit(): Boolean {

        var isValid = true

        if (metric_unit_weight_et.text.toString().isEmpty() ||
            metric_unit_height_et.text.toString().isEmpty()) {
            isValid = false
        }
